...
public partial class UniWebView : MonoBehaviour
{
    ...
    //Comando removido - WebView deve permanecer sempre aberto
    // public void CloseWebView() - REMOVIDO para manter WebView sempre aberto

    public void LoadNewUrl(string newUrl)
    {
        if (!string.IsNullOrEmpty(newUrl))
        {
            OpenWebView(); // Abre o WebView
            Load(newUrl);
        }
    }
    ...
    void Update()
    {
        ...
        if (backDetected)
        {
            UniWebViewLogger.Instance.Info("Received Back button, but WebView should remain open - ignoring close request.");
            if (CanGoBack)
            {
                GoBack();
            }
            ...
            {
                // WebView deve permanecer aberto - não fechar
                UniWebViewLogger.Instance.Info("Back button pressed but WebView will remain open as requested.");
            }
        }
    }
    ...
    void OnDisable()
    {
        // WebView deve permanecer sempre visível - não esconder
        if (started)
        {
            UniWebViewLogger.Instance.Info("OnDisable called but WebView will remain visible as requested.");
            // _Hide(useAsync: true); // REMOVIDO para manter WebView sempre aberto
        }
    }
    ...
    {
        UniWebViewInterface.Load(listener.Name, "about:blank", false, null); // Carrega uma página em branco
        _Hide(true, UniWebViewTransitionEdge.None, 0.4f, false, null);
    }
